...
<SandpackPlayground
  client:only="react"
  title="Live example · Multi-Database Orchestration"
  entry="/after.ts"
  files={{
    "/before.ts": `// ── Before: one shared instance for every project ─────────
// Three projects share a single database server and its memory
// pool. Nothing bounds what one project can consume, and the
// server has a single lifecycle.
...
const workloads: Workload[] = [
  { project: 'project_a', memoryMb: 180 },
  { project: 'project_b', memoryMb: 220 },
  { project: 'project_c', memoryMb: 90 },
];

function used() {
  return workloads.reduce((sum, w) => sum + w.memoryMb, 0);
}

function status() {
  for (const w of workloads) {
    const ok = running && used() <= POOL_MB;
    console.log(\`  \${w.project}: \${ok ? 'healthy' : 'UNAVAILABLE'}\`);
  }
}

console.log(\`── steady state (\${used()}/\${POOL_MB} MB)\`);
status();

// project_b runs an unbounded analytical query.
workloads[1].memoryMb += 900;
console.log(\`\\n── runaway query in project_b (\${used()}/\${POOL_MB} MB)\`);
status();

// Recover by restarting the server, which stops everyone.
workloads[1].memoryMb = 220;
running = false;
console.log('\\n── restart for project_b maintenance');
status();
console.log('\\none project\\'s problem became an outage for all three');
`,
    "/after.ts": `// ── After: one instance per project ───────────────────────
// Each project gets its own container with a memory limit and
// a health check. Faults and lifecycle operations are scoped to
// a single instance.

type Health = 'healthy' | 'unhealthy' | 'stopped';
...
class DatabaseInstance {
  ...
  constructor(
    ...
    this.usedMb = baselineMb;
  }

  allocate(mb: number) {
    if (this.usedMb + mb > this.limitMb) {
      console.log(\`  \${this.project}: query cancelled, \${mb} MB exceeds \${this.limitMb} MB limit\`);
      return;
    }
    this.usedMb += mb;
  }
  ...
  // Stands in for pg_isready: running is not the same as ready.
  health(): Health {
    if (!this.running) return 'stopped';
    return this.usedMb <= this.limitMb ? 'healthy' : 'unhealthy';
  }
}

const cluster = [
  new DatabaseInstance('project_a', 5432, 512, 180),
  new DatabaseInstance('project_b', 5433, 512, 220),
  new DatabaseInstance('project_c', 5434, 256, 90),
];

function status() {
  for (const db of cluster) {
    console.log(\`  \${db.project} :\${db.port} \${db.health()}\`);
  }
}
...
console.log('\\n── runaway query in project_b');
cluster[1].allocate(900);
status();

console.log('\\n── maintenance on project_b only');
cluster[1].stop();
status();
cluster[1].start();

const healthy = cluster.filter((db) => db.health() === 'healthy').length;
console.log(\`\\n\${healthy}/\${cluster.length} instances healthy; the fault never left project_b\`);
`,
  }}
/>
