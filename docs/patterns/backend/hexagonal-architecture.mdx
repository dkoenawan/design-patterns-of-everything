...
<SandpackPlayground
  client:only="react"
  title="Live example · Hexagonal Architecture"
  entry="/after.ts"
  files={{
    "/before.ts": `// ── Before: core coupled to infrastructure ──────────────────
// PlaceOrderUseCase constructs and calls Postgres directly.
// Testing the use case means standing up a real database.

class PostgresOrderRepository {
  private rows: Record<string, any> = {};

  save(order: { id: string; items: string[] }): void {
    console.log(\`[Postgres] INSERT INTO orders VALUES ('\${order.id}')\`);
    this.rows[order.id] = order;
  }

  findById(id: string) {
    console.log(\`[Postgres] SELECT * FROM orders WHERE id = '\${id}'\`);
    return this.rows[id] ?? null;
  }
}

class PlaceOrderUseCase {
  private orders = new PostgresOrderRepository(); // concrete — not swappable

  execute(id: string, items: string[]): void {
    this.orders.save({ id, items });
  }
...
useCase.execute("ORD-001", ["Widget"]);
console.log(useCase.lookup("ORD-001"));
`,
    "/after.ts": `// ── After: Hexagonal Architecture ───────────────────────────
// The core depends on an OrderRepository port, not a concrete
// database. Adapters implement the port; the core never changes.

interface OrderRepository {
  save(order: { id: string; items: string[] }): void;
  findById(id: string): { id: string; items: string[] } | null;
}

// Adapter — owns the infrastructure detail
class PostgresOrderRepository implements OrderRepository {
  private rows: Record<string, any> = {};
...
// Adapter — no infrastructure required, ideal for tests
class InMemoryOrderRepository implements OrderRepository {
  private rows: Record<string, any> = {};

  save(order: { id: string; items: string[] }): void {
    console.log(\`[InMemory] storing '\${order.id}'\`);
    this.rows[order.id] = order;
  }

  findById(id: string) {
    console.log(\`[InMemory] looking up '\${id}'\`);
    return this.rows[id] ?? null;
  }
}
...
const prodUseCase = new PlaceOrderUseCase(new PostgresOrderRepository());
prodUseCase.execute("ORD-001", ["Widget"]);
console.log(prodUseCase.lookup("ORD-001"));

// Wired with in-memory adapter — no infrastructure required:
const testUseCase = new PlaceOrderUseCase(new InMemoryOrderRepository());
testUseCase.execute("ORD-002", ["Gadget"]);
console.log(testUseCase.lookup("ORD-002"));
`,
  }}
/>
