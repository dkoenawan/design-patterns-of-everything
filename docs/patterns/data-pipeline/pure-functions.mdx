...
<SandpackPlayground
  client:only="react"
  title="Live example · Pure Functions"
  entry="/after.ts"
  files={{
    "/before.ts": `// ── Before: impure transform ───────────────────────────────
// Reads from a "database," mutates a shared running total, and
// filters — all tangled in one function. Untestable without a
// fake DB, and calling it twice gives different results.
...
// Simulates an external data source
function readOrdersFromDb(): Order[] {
  return [
    { id: 'ORD-1', total: 120 },
    { id: 'ORD-2', total: 45 },
    { id: 'ORD-3', total: 210 },
...
let runningTotal = 0; // shared mutable state — a hidden dependency

function processHighValueOrders(threshold: number): Order[] {
  const orders = readOrdersFromDb(); // hidden I/O dependency
  const result: Order[] = [];
  for (const o of orders) {
    if (o.total > threshold) {
      runningTotal += o.total; // mutates external state as a side effect
      result.push(o);
    }
  }
  return result;
  // Problem: calling this twice doubles runningTotal. Unit-testing
  // requires mocking readOrdersFromDb. The result depends on when
  // and how many times this function has run before.
}

console.log('High-value orders:', processHighValueOrders(100));
console.log('Running total (hidden side effect):', runningTotal);
`,
    "/after.ts": `// ── After: pure transform, I/O pushed to the boundary ─────
// The transformation takes plain data and returns plain data.
// No hidden reads, no mutation of external state — every call
// with the same input gives the same output.
...
// I/O lives at the boundary, separate from the transformation
function readOrdersFromDb(): Order[] {
  return [
    { id: 'ORD-1', total: 120 },
    { id: 'ORD-2', total: 45 },
...
// Pure — just the transformation. Deterministic, no side effects.
function filterHighValue(orders: Order[], threshold: number): Order[] {
  return orders.filter((o) => o.total > threshold);
}

// Pure — derives a total without mutating anything
function sumTotals(orders: Order[]): number {
  return orders.reduce((sum, o) => sum + o.total, 0);
}

const orders = readOrdersFromDb();
const highValue = filterHighValue(orders, 100);
const total = sumTotals(highValue);

console.log('High-value orders:', highValue);
console.log('Total (derived, not mutated):', total);

// Calling the same pure functions again with the same input
// yields the exact same result — no hidden state to worry about.
console.log('Re-run is identical:', filterHighValue(orders, 100).length === highValue.length);
`,
  }}
/>
...
- [Batch vs Streaming](./batch-vs-streaming.mdx) — pure transformation logic is execution-model agnostic: the same functions run in batch Spark jobs and streaming Flink operators
