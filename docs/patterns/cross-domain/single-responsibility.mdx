...
- [CQRS](../backend/cqrs.mdx) — SRP at the model level: the command model and read model each have exactly one reason to change
- [Dependency Injection](../backend/dependency-injection.mdx) — DI enforces SRP by externalizing infrastructure concerns from the class that uses them
...
- [Micro-Frontend Architecture](../frontend/micro-frontend-architecture.mdx) — SRP applied at the deployment boundary: each remote team owns exactly one business domain
